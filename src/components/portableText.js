...
const PortableText = ({ blocks, className, trackSectionHeader }) => {
  useEffect(() => {
    ...
    document.querySelectorAll('main a').forEach(anc => {
      if(anc.getAttribute('href').includes('#')) tempAnchors.push(anc);
    })
    ...
    tempAnchors.forEach(a => {
      a.addEventListener('click', function (e) {
          ...
          let elem = document.getElementById(href)||document.querySelector("a[name="+href.substring(1, href.length)+"]");
