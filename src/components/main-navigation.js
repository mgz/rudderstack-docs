...
const MainNavigation = () => {
  ...
  return (
    <>
    <nav className="container mx-auto px-3 flex items-center justify-between flex-wrap py-6 ">
      ...
      <div className={`${ isExpanded ? `block` : `hidden` } w-full block justify-content-center lg:flex lg:items-center lg:w-auto`}>
        <div className=" lg:flex-grow">
        {
          mainmenu.map(
              (link, i) => (
                  <Link to={link.menu_item_link} className="block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4">
        ...
        <div>
          ...
          <Link to={trybtn.btnlink}><span className="inline-block p-3 text-white bg-black normal-case">{trybtn.btntext}</span></Link>
        </div>
      </div>
    </nav>
        {/* <nav className="flex">
