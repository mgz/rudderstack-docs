      ...
      latestblogs.map(
          (latestblog, i) => (
              <div key={i} className="w-full md:w-1/3 p-4 flex flex-col flex-grow flex-shrink">
                <div className="border border-grayColor-priceCardBorder rounded-3xl shadow-md overflow-hidden">
                  <Link to={`blog/`+latestblog.node.slug}>
                    <div className="flex-1 bg-white rounded-t rounded-b-none overflow-hidden shadow">
                        ...
                        <div className="w-full text-lg md:text-2xl leading-6 md:leading-7 font-bold pt-6 px-5 blog-title-wrap">{latestblog.node.title}</div>
                    </div>
                  </Link>
