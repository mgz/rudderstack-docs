...
There are limits on the number of custom properties per event that can be sent to Google Analytics 4. RudderStack SDK <code class="inline-code">doesn't drop</code> the payload based on these limits. If the events are not showing up in <strong>debug view or reports on Google Analytics dashboard</strong>, please refer <a href="https://support.google.com/analytics/answer/9237506?hl=en">this</a> for collection limits.
