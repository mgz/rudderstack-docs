...
<div class="infoBlock">
  ...
  RudderStack uses the <a href="https://developer.zendesk.com/rest_api/docs/sunshine/events_api">Zendesk Sunshine Events
  API</a> for the 
  <code class="inline-code">track</code> calls. To successfully send the <code class="inline-code">track</code>
  calls to Zendesk, the following criteria should be met:
  <ul>
    <li>You need to have signed up for the Events API early access program</li>
    <li>A <code class="inline-code">userId</code> needs to be passed along with the <code class="inline-code">track</code> call</li>
  </ul>
</div>
