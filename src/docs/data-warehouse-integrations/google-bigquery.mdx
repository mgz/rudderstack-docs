...
[**GitHub
repo**](https://github.com/rudderlabs/rudder-transformer/tree/master/v0/destinations/bq)**.**
{% endhint %}
...
{% hint style="success" %}
If a dataset with the name \(configurable by the setting: **Namespace** in the
RudderStack destination settings\) already exists, the role of **`BigQuery Data Editor`** would suffice instead of **`BigQuery Data Owner`**.
{% endhint %}
