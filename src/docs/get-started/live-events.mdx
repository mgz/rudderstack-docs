...
To use this feature, make sure you have configured your source and destination in RudderStack. For more information on adding a source and destination in RudderStack, follow this <a href="../connections/adding-source-and-destination-rudderstack">guide</a>.
=======
To use this feature, make sure you have configured your source and destination in RudderStack. For more information on adding a source and destination in RudderStack, follow this <a href="../../connections/adding-source-and-destination-rudderstack/">guide</a>.
...
For details on the information available in the <strong>Live Events</strong> window, check the <strong>Source Live Events</strong> section.
...
- Make sure you have configured a source and destination in RudderStack. Follow this [**guide**](../../connections/adding-source-and-destination-rudderstack/) for more information.

<div class="warningBlock">
<<<<<<< HEAD

Make sure your destination is configured in the <strong>Cloud mode</strong>. Refer to the <a href="../connections/rudderstack-connection-modes">Connection Modes</a> guide for more information on the Cloud mode.
=======
Make sure your destination is configured in the <strong>Cloud mode</strong>. Refer to the <a href="../../connections/rudderstack-connection-modes/">Connection Modes</a> guide for more information on the Cloud mode.
>>>>>>> origin/docs_migrate_gp
...
For details on the information available in the <strong>Live Events</strong> window, check the <strong>Destination Live Events</strong> section.
...
As seen above, you were trying to send an `identify` event to Facebook Custom Audience, which the destination does not support. You can refer to the [**Custom Audience documentation**](../../destinations/advertising/fb-custom-audience) to confirm that only `track` events are supported. As a result, when RudderStack tries sending the `identify` event to the destination, an error is thrown. RudderStack tries sending this event several times before marking it as aborted.
...
Live events are shown for all the destinations. However, the <strong>payload of the events</strong> is not shown for some destinations like the <a href="../destinations/storage-platforms">object storage platforms</a> and the <a href="../data-warehouse-integrations">data warehouses</a> supported by RudderStack.
=======
Live events are shown for all the destinations. However, the <strong>payload of the events</strong> is not shown for some destinations like the <a href="../../destinations/storage-platforms/">object storage platforms</a> and the <a href="../../data-warehouse-integrations/">data warehouses</a> supported by RudderStack.
>>>>>>> origin/docs_migrate_gp
...
To know more about the Live Events Debugger, you can [**contact us**](mailto:%20docs@rudderstack.com) or start a conversation on our [**Slack**](https://rudderstack.com/join-rudderstack-slack-community) channel.
